...
public class ScrapingRockdelux extends AbstractWebScraping {
    ...
    public ScrapingRockdelux(String rutaDestino) {
        super("Rockdelux", "http://www.rockdelux.com/discos/albumes/page",
                "http://www.rockdelux.com/discos/p/", 12, rutaDestino);
    ...
    public void scrappingWeb(String url, String subUrl, int numPages,
        ...
        for (int iNumPage = 1; iNumPage <= numPages; iNumPage++) {
            ...
            try {
                ...
                for (Element link : links) {
                    ...
                    if (strLink.contains(subUrl)) {
                        if (!link.text().equals("")) {
                            ...
                            logger.info(tipoText[index] + ": "
                                    + link.text().trim());

                            if (index > 0) {
                                album.setTitulo(link.text().trim());
                                logger.info("T’tulo album: "
                                        + album.getTitulo());
                                ...
                                try {
                                    album.setFecha(listEtiquetas.get(1)
                                            .substring(0, 4));
                                    logger.info("Anyo album: "
                                            + album.getFecha());
                 ...
                 */
                TratarCSVAlbum.generarCSVAlbums(listaAlbums, numMaxEtiquetas,
                        destinyPath, logger);
            } catch (IOException e) {
