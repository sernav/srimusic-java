 ...
 * recientemente por un usuario de la aplicación web LastFm. Se utiliza el
 * proyecto lastfm-java que se comunica con la aplicación LastFm mediante los
...
public class PlaybackDUsuario implements IAnalisisLastfm {
    ...
    /**
     * Inicia la sesión de usuario mediante la key de LastFm para Sri.Landa y su
     * clave secreta.
     * 
    ...
    public void run() {
        try {
            ...
            e.printStackTrace();
            Log.log(e, "("+ PlaybackDUsuario.class.getSimpleName() + ") Excepción LastFm! " + e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
            Log.log(e, "("+ PlaybackDUsuario.class.getSimpleName() + ") Excepción General! " + e.getMessage());
        } finally {
